class Ant{
  PVector pos ;
  float dirAngle = int(random(nbBranches))*TAU/6 + TAU/12 ;
  color colour ;
  ...
  void paint(){
    ...
    beginShape() ;
    for( float angle = 0 ; angle <= TAU+EPSILON ; angle += TAU/nbBranches ){
      vertex( pos.x + centerVertexDistance*cos(angle) , pos.y + centerVertexDistance*sin(angle) ) ;
      vertex( pos.x + innerRadius*cos(angle+TAU/nbBranches/2) , pos.y + innerRadius*sin(angle+TAU/nbBranches/2) ) ;
    }
    endShape() ;
    pg.beginShape() ;
    for( float angle = 0 ; angle <= TAU+EPSILON ; angle += TAU/6 ){
      ...
      pg.vertex( pos.x + innerRadius*cos(angle+TAU/nbBranches/2) , pos.y + innerRadius*sin(angle+TAU/nbBranches/2) ) ;
    }
    pg.endShape() ;
  }
