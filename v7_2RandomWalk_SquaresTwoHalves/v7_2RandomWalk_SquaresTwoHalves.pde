...
void setup() {
  ...
  for( int k = 0 ; k < NB_OF_ANTS ; ++k ){
    ...
    ants.add( new Ant( int(random(widthCanvas/SIZE_OF_A_SQUARE))*SIZE_OF_A_SQUARE+SIZE_OF_A_SQUARE/2 , int(random(widthCanvas/SIZE_OF_A_SQUARE))*SIZE_OF_A_SQUARE+SIZE_OF_A_SQUARE/2 , c1 , c2 ) ) ;
  }
}
...
void keyPressed(){
  ...
  if( key == 'c' ){
    ...
    pg.background(0) ;
    for( Ant ant : ants ){
      ant.colour1 = randomColor() ;
      ant.colour2 = randomColor() ;
    }
  }
}
